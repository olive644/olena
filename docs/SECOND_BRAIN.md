...
Política de Privacidade: `public/politica-de-privacidade.html` (versão em `src/domain/privacy-policy.ts`, conferida por `src/privacy-policy-page.test.ts`). O login, no fim do onboarding, exige a concordância antes de habilitar "Entrar com Google" e grava a versão aceita em `helena.privacy.v1`. Ao adicionar um serviço externo (host novo no `connect-src` do `vercel.json`), atualize a política e a versão; o teste falha enquanto o serviço não estiver citado.
