...
- **Rodapé da marca** (`BrandFooter`): "Todos os direitos Galeria.Oli - OlenaStudy", com o ícone da Galeria.Oli à esquerda, no onboarding, no login e na política. O ícone é `public/galeria-oli-icon.png` (256 por 256, PNG com transparência, sem o fundo branco da arte original).
...
- **Travas contra política desatualizada** (`src/privacy-policy-page.test.ts`): cada serviço externo liberado no `connect-src` do CSP precisa estar coberto, em palavras simples, na política; termos técnicos e nomes de fornecedores são proibidos; a empresa, a equipe, o rodapé e o ícone (PNG quadrado com transparência) são conferidos. Se um serviço novo entrar no CSP, o teste falha até a política ser atualizada.
